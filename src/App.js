...
class App extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      this.state.usState !== prevState.usState
    ) {
      this.setState({ selectedRecord: null });

      if (this.state.type && this.state.usState) {
        axios
          .get(`http://localhost:3000/${this.state.type}/${this.state.usState}`)
          .then(res => {
          ...
          .catch(err => {
            if (err) {
              window.UIkit.notification({
                message:
                  "Sorry! We weren't able to get the results. Please try again later",
                status: "danger"
              });
            }
          });
  ...
  render() {
    return (
      <div className="table">
        <h1 className="header">Who is your representative?</h1>
        <div className="center">
          <form className="flex-table">
            <div>
                        ...
                        return (
                          <tr
                            ...
                            className={
                              ...
                              record.name === this.state.selectedRecord.name
                                ? "gray"
                                : ""
                            }
                            onClick={e => {
